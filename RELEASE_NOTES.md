...
- Adding a programme occurrence can explicitly reuse an existing UserExercise; users can still create distinct custom exercises with identical visible names without merging their history.
...
- Friend comparison is exact-canonical-match, user-opt-in per programme occurrence, and e1RM-only. Epley+RIR v1 qualifying rules are shared with Android. Link/unlink and opt-in changes now reconcile the complete eligible local history atomically so old workouts backfill and stale comparison points are removed.
