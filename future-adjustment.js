...
export function evaluateFutureAdjustment(exercise,sets){
  ...
  if(exercise.equipment===Equipment.BODYWEIGHT||(!severe&&Number(exercise.targetReps)>1)){
    const amount=severe&&failures.length>=2?2:1;
    return {kind:'REDUCE_REPS',suggestedReps:Math.max(1,Number(exercise.targetReps||1)-amount),failureCount:failures.length,severe,reason:failures.length>=2?'Several sets reached failure. A slightly lower rep target next time may make the exercise more sustainable.':'This exercise reached failure. A slightly lower rep target next time may fit your current capacity better.'};
  }
  ...
  reduced=snapSelectable(Math.max(0,reduced),exercise.equipment||Equipment.OTHER);
